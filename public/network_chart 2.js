...
function triggerScraper(url) {
    console.log("Triggering scraper for URL:", url);
    fetch(`/api/trigger-scraper?url=${url}`)
        .then(response => response.json())
        .then(data => {
            // Since the scraper is now running asynchronously, we'll get an immediate response
            // indicating that the scraper has started.
            if (data.status === 'success') {
                displayMessage(data.message, 'green'); //green for success
                displayLogs(data.logs);  // New function to display logs
            } else {
                displayMessage(data.message, 'red');
            }
        })
            // If you implement a mechanism to notify the frontend when scraping is complete,
            // you can handle that here (e.g., using WebSockets or Server-Sent Events).
       
        .catch(error => {
            console.error("Error fetching:", error);
            displayMessage('An error occurred while triggering the scraper.', 'red');
        });
}
function displayLogs(logs) {
    const logContainer = document.getElementById('logContainer'); // Assuming you have a div with id 'logContainer' to display logs
